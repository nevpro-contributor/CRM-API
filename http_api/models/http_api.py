...
import json, ast, urllib3, json
# from bzrlib.plugins.bash_completion.bashcomp import kwargs
...
class TestAPI(models.Model):
    ...
    pending_amt=fields.Integer('Pending Amount')
    bill_no = fields.Integer('Bill Number')
    ...
    def create_record(self,**kwargs):
          ...
          if (kwargs['user'] == admin) and (kwargs['password'] == admin):
              return kwargs['user']
              for data in datas:
                  self.env['test.api'].create({
                           'customer': self.env['res.partner.search'].search([('notes_user_id', '=', data['customer'])]).id,
                           })
